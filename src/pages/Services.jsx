...
export default function Services() {
  return (
    <section id="sistem" className="bg-hijau-900 py-24 md:py-32">
      <div className="container mx-auto px-6">
        <div className="max-w-2xl mb-16">
          ...
          <h2 className="judul-besar text-judul-1 text-gading-050 mb-6">
            ...
            jalan <span className="text-emas">24 jam</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 grid sm:grid-cols-2 gap-px bg-hijau-700 border border-hijau-700">
            {SISTEM.map((s) => (
              <div
                key={s.judul}
                className="bg-hijau-900 p-8 hover:bg-hijau-800 transition-colors"
              >
                <h3 className="judul text-xl text-gading-050 mb-3">{s.judul}</h3>
                <p className="text-gading-300 leading-relaxed">{s.isi}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-gading-050 text-hijau-900 p-8">
            <h3 className="judul text-xl text-hijau-500 mb-8 pb-4 border-b-2 border-hijau-500">
              Perangkat keras
            </h3>
            <ul className="space-y-5">
              {PERANGKAT.map((p) => (
                <li
                  key={p}
                  className="flex items-start gap-4 pb-5 border-b border-gading-100 last:border-0"
                >
                  <span
                    className="mt-1.5 w-2 h-2 bg-merah rounded-full shrink-0"
                    aria-hidden="true"
                  />
